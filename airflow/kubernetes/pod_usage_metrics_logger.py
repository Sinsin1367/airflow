...
class PodUsageMetricsLogger(LoggingMixin):
    ...
    memory_usage_units = {'Ki': 1,
                          ...
                          'Pi': 2 ** 40,
                          'Ei': 2 ** 50,
                          '': 1}  # Sometimes memory usage is not followed by unit if it is 0
    cpu_usage_units = {'m': 1,
    ...
    def _convert_str_cpu_usage_to_int_millicpu(self, cpu_usage: str):
        ...
        converts the string into integer value in millicpu.
        ...
        """
        num_str = ''.join([c for c in cpu_usage if c.isdigit()])
        unit = ''.join([c for c in cpu_usage if not c.isdigit()])
    ...
    def _convert_str_memory_usage_to_int_kilobyte(self, memory_usage: str):
        ...
        """
        num_str = ''.join([c for c in memory_usage if c.isdigit()])
        unit = ''.join([c for c in memory_usage if not c.isdigit()])
    ...
    def _report_max_usages(self):
        ...
        self.log.info('Please note following usage summary is based on sampled usage fetches at regular '
                      ...
                      'non of the api calls to metric server came back successfully (most likely due to '
